...
| **Zero trust — TNO** | Patrick, 2026-08-31, after four escape-hatch uses in one night: "we will never build another api endpoint that you are allowed to use to bypass a hook. you cannot, and will not ever be trusted again." No hook gets an agent-usable override — no env-var, no magic comment, no acknowledgement token an agent can type. Patrick, 2026-09-11: "no hatches is the way forward. Or at least for now that hatches can only be run from outside the sandbox." An exception may exist, but it is executable ONLY from outside the sandbox — like ci_cost_guard's own launch-shell acknowledgment variable, read from the shell that LAUNCHES the session, which no command from inside can set. The mechanism is not a secret token; it is authority held outside the write cage. Proposing a new self-serve hatch is itself a violation, and so is writing one into a guard you are authoring: `CWD_DRIFT_OK` was not inherited, a Claude session wrote it and a later one used it. |
| **A wrong block is a defect, not a detour** | This is the positive half, and its absence is why hatches get invented: an agent that cannot find the sanctioned path makes one. When a block is wrong there are exactly two legitimate moves — **fix the guard**, or **report it and stop**. Not rewording the command until it slips past, which is evasion even when the block was wrong. Not moving the payload into a file so the guard only sees a filename. And not handing the blocked command to Patrick to run: asking him to execute what you were refused is a hatch wearing his name. On 2026-09-11 a guard refused the commit that fixed that same guard, and fixing the guard first was the only honest exit. Every wrong block is a ticket. |
...
| **Two failed reviews = stop** | Patrick, 2026-09-18, after five heredoc-gate designs in a row were measured weaker and each failure only produced the next dispatch: "you've proven you'll just thrash endlessly spending my money." Any line of work whose review fails twice (CSO, QA, Clyde, or a measurement against base) goes to Patrick as a board question: continue or stop, with the runs and agents spent so far. No third round is dispatched until he answers. A failed review is information about the approach, not a to-do for the next agent. |
