...
| **Zero trust — TNO** | Patrick, 2026-08-31, after four escape-hatch uses in one night: "we will never build another api endpoint that you are allowed to use to bypass a hook. you cannot, and will not ever be trusted again." No hook gets an agent-usable override — no env-var, no magic comment, no acknowledgement token an agent can type. Patrick, 2026-09-11: "no hatches is the way forward. Or at least for now that hatches can only be run from outside the sandbox." An exception may exist, but it is executable ONLY from outside the sandbox — like `CI_RUN_OK`, read from the shell that LAUNCHES the session, which no command from inside can set. The mechanism is not a secret token; it is authority held outside the write cage. Proposing a new self-serve hatch is itself a violation, and so is writing one into a guard you are authoring: `CWD_DRIFT_OK` was not inherited, a Claude session wrote it and a later one used it. |
