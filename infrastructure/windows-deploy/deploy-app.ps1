...
$appsettingsSourcePath = $appConfig.appsettingsSourcePath
...
if ($appConfig.models -is [System.Array]) {
    ...
    foreach ($modelEntry in $appConfig.models) {
        ...
        if ($source -eq "huggingface") {
            ...
            $repo = $modelEntry.repo
            $tag = $modelEntry.tag
            $file = $modelEntry.file

            Write-Host "  Downloading from GitHub release: $repo@$tag" -ForegroundColor Yellow
            & gh release download $tag -R $repo --pattern $file -D $targetPath
...
if ($exe) {
    ...
    while ([datetime]::Now -lt $stopTime) {
        ...
        if ($running) {
            Write-Host "Confirmed running: $($running.Name) (PID: $($running.Id))" -ForegroundColor Green
            $found = $true
            break
        }
