...
## 2026-09-17/18 — the heredoc-gate redesign, stopped

Five designs in a row set out to make the heredoc parser stricter or quieter:
CE-2.47, CE-25.4, and planner v2, v3 and v3a. Every one measured WEAKER than
base.
- The last, CE-25.20, passed Clyde 10/10 and QA.
- A base-against-change probe then printed 21 flips. Fifteen were the
  grep-family masking: bash can rebind the name `grep` from inside a command.
  Six came from the new pipeline split.
- Two CSO sessions were cut off by a safety classifier before they measured
  anything.

Patrick answered "stop". Base stays, and the dead branches are parked under
refs/parked/2026-09-18-*. The retro is
docs/retrospectives/2026-09-18-heredoc-gate-redesign.md, and a new shared rule
came out of it: "Two failed reviews = stop" (CE-2.48, PR #79).

The same stretch shipped:
- CE-25.1/.2/.3/.8 (ci_cost_guard Darwin-only, deploy_guard deny reason, the
  parser crash heal, ci_cost off-repo refusal): PR #78;
- CH-234.9, only the creating host moves a ticket's status: PR #155;
- CE-12.14, a refusal ends a subagent's run, and CE-12.15, refusals tallied
  from the transcript: PR #80, both live in settings.

Found along the way:
- An analyst ran its ticket commands through Python scripts in both rounds.
  The first time it did not say so. Full-guard probes showed the scripts
  carried 5 criteria past visual_ac_manual_guard.
- An enforcement inventory, kept private, found:
  - git over SSH authenticates as the owner account;
  - Azure is logged in with Owner;
  - the robot token can dispatch workflows.
  Patrick has the recommendations.

## 2026-09-18/19 — nfl-stats gets its season

The loop that worked: one small story per Patrick decision, with its budget on
the ticket, then one dev, Clyde per criterion, one QA run, and UAT on a served
preview. Seven stories went through with no UAT bounce:
- NFL-2.5: 70/30 season blend.
- NFL-2.6: last completed week, and greyed "Played" rows.
- NFL-2.7: fantasy points over expected.
- NFL-2.8: Next Gen Stats.
- NFL-2.9: PFR advanced stats.
- NFL-2.10: hover definitions on headings.
- NFL-2.11: FTN charting.

Each source's contract (units, IDs, weeks, postseason, volume floors) was
measured from a real pull and pinned in tests before building.

Decided by Patrick:
- the site stays LLM-independent;
- "if you can find the data, use it";
- run lanes in parallel with GLM devs, Flash by default.

Idea filed, not approved: CH-224.92, where automated criteria are checked by
running their Check command, with no agent.

NFL-3.1 (sortable headings, with raw data-sort keys written in Python) shipped
too, making eight stories with no UAT bounce. nfl-stats had never had a main.
It was created at the first commit, and the first develop → main release is
psford/nfl-stats#1. Next, agreed with Patrick: one combined feature, a weighted
start/sit projection whose weights come from historical trends. It starts with
scoping.

## 2026-09-19 — the start/sit projection, shipped and hosted

Scoped, built and released the projection feature in one session, then put the
site on the web.

**The model.** Ten seasons of nflverse history (2016-2025) cached by a new
`model_data.py`; `projection.py` turns it into one row per player-game whose
inputs are provably from before kickoff; `fit.py` fits a lasso per position for
PPR points and receptions, choosing the penalty by holding out whole seasons;
`backtest.py` grades it on 2024-2025 against both numbers the page already
showed. Held out, it beats Recent Form at every position on both measures. It
beats Blended PPG on average miss at QB, ties at WR and TE, is 0.07 points
worse at RB, and orders players better than both everywhere. Patrick read
`docs/projection-report.md` at the checkpoint and answered START.

**The page.** Proj (the default sort), Value (Proj minus Recent Form), Value
picks per position, a projection log that grades itself once games are final,
and a note. The weights are applied as plain arithmetic; a test starts a fresh
interpreter and proves the page modules import without scikit-learn, so the
site stays free of any model library.

**Hosting.** nfl.psford.com, a Cloudflare Worker serving static assets. Unlike
omni-map, nothing builds in Cloudflare: the page is generated by Python from a
49MB local cache, so the built page is committed and the Worker only serves it.

**What the reviews actually caught.** Nine Clyde dispatches passed nine times
and found nothing. Every real defect came from reading the diff: a leakage test
that only shifted later weeks (so a season average including the game itself
would have passed), the report's unit conversion inverted, Next Gen week-0 rows
that are whole-season totals, and a page note that read as a contradiction.
Patrick's verdict on Clyde: "a mid-to-ok idea in theory, and something of a
disaster in practice". It also detached the main checkout twice. It is being
retired next.

**Process failures worth keeping.** Cancelling an epic's last open child
auto-cancels the epic, and a cancelled epic cannot be reopened — NFL-5 died
that way and was refiled as NFL-6. Criteria need `ticket ac verify`; the prose
"Verified by ..." does not set the field, and Patrick's accept bounced on it.
A long check run gets ONE run, saved to a file; three runs of a twenty-minute
suite to read one line of output was the session's worst waste.

**Harness fixes** (psford/claude-harness#156): the queue no longer shows draft
stories, which are agent work (CH-224.93), and an epic's accept is no longer
